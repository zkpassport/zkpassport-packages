...
import { CircuitRootCard } from "@/components/CircuitRootCard"
import { LoadingAnimation } from "@/components/LoadingAnimation"
import { useHistoricalCircuitRoots } from "@/hooks/useHistoricalCircuitRoots"
import { Suspense } from "react"

function HistoricalRootsContent() {
  const { roots, isLoading, error } = useHistoricalCircuitRoots()
    ...
    <div className="space-y-6">
      {isLoading && roots.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {roots.map((root) => (
            <CircuitRootCard key={root.root} rootDetails={root} />
          ))}
        </div>
      )}
    </div>
...
export default function HistoricalRootsPage() {
  return (
    <div className="container mx-auto py-6 px-4 sm:py-10">
      <h1 className="text-xl sm:text-2xl font-bold mb-2">Historical Circuit Roots</h1>
      <p className="text-muted-foreground mb-6 text-sm sm:text-base">
        Browse all historical circuit roots stored in the registry
      </p>
