...
const FIXTURE: Fixture = JSON.parse(
  readFileSync(
    new URL("../../test-harness/fixtures/outer_count_5_age.json", import.meta.url),
    "utf8",
  ),
)
...
async function main() {
  ...
  const [{ contract: registry }] = await timed("deploy registry", () =>
    ZKPassportRegistryContract.deploy(
      wallet,
      admin,
      oracle,
      admin,
  ...
  await timed("seed certificate root", () =>
    registry.methods
      .update_root(
        ...
        Fr.fromHexString(FIXTURE.certificateRegistryRoot),
        0n,
        validFrom,
      )
  ...
  await timed("enable version", () =>
    registry.methods
      .set_version_status(CIRCUIT_MANIFEST_VERSION, true)
      .send({ from: admin, wait: WAIT }),
  )
  ...
  let doubleError = ""
  try {
    await ageGate.methods
      .claim(boundUser)
      .send({ from: user, capsules: [capsule(blob)], wait: WAIT })
  } catch (e) {
    doubleFailed = true
  ...
  console.log(
    `time strategy: node debug warpL2TimeAtLeastBy(${bump}s) to fixture-anchored target, L2 ts ${before} -> ${after}`,
  )
  ...
  if (!doubleFailed) {
    console.error(
      "FAILED: the double claim was not rejected — the uniqueness nullifier did not collide.",
    )
    process.exit(1)
  }
