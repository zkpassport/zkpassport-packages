...
|---|---|---|---|
| `zkpassport_registry_contract` | `contract` | The registry. Roles (`admin`/`oracle`/`guardian`) are plain `PublicMutable` — writes take effect instantly (`main.nr:24-26`). Everything else — `paused`, per-registry root sets (certificate/circuit/sanctions) with validity windows and revocation, the accepted-VK set, and the OPRF pubkey hash — is `DelayedPublicMutable` (24h delay). Exposes `update_root`/`batch_update_roots` (oracle), admin policy setters, and private `#[view]` functions (`assert_proof_valid`, `assert_sanctions_root_valid`, `assert_root_valid_at_timestamp`) that verifiers call. | `aztec` (git, tag `v5.2.0`) |
| `zkpassport_core` | `lib` | Pure, no-`aztec`-dependency core: outer-proof recursion (`verify_outer_proof_core`), public-input parsing, and commitment wrappers (`age_commitment`, `disclose_commitment`, `sanctions_commitment`, `bind_user_address_commitment`) over the ZKPassport circuits' own Noir libs. This crate is what the `harness/` proves against natively (no TXE needed). | `bb_proof_verification` (git, tag `v5.2.0`), `poseidon` (git, tag `v0.3.0`), plus **path deps** into the sibling `circuits` checkout — see Pins below |
...
2. **In M1, roots are oracle-pushed, not bridged.** `update_root`/`batch_update_roots` are
   `#[external("public")]` functions gated on an `oracle` role address set by `admin`; there is
   no L1→L2 message consumption yet. M2 scope is an L1→L2 bridge that feeds the same
   `update_root`/`batch_update_roots` write path, so the private verify path (and this library)
...
4. **TXE/`aztec-nargo test` no-op real recursion.** The `verify_proof_with_type`/
   ...
   control flow and storage semantics only. **`bb verify` (the `harness/`) and the sandbox e2e
...
#    recursive verification) — from harness/, several minutes:
cd ../harness && ./run-harness.sh fixtures/outer_count_4_disclose.json
```
...
suite — a fresh fixture (see below). Note step 4 above leaves you in `harness/`; `e2e/` is a
sibling of `harness/` and `zkpassport.nr/` under `packages/aztec/`.
...
`harness/fixtures/*.json` and `examples/age_gate_contract/src/test/fixtures.nr` embed a real
...
Regenerate via `harness/gen-fixtures.sh [disclose|age]`, then `harness/fixture_to_noir.py` to
...
| `@zkpassport/utils` (TS) | `0.37.4` (pinned in the `circuits` repo's own `package.json`) | Used by `harness/fixture-gen.ts` and `harness/golden-vectors.ts`, both of which run with cwd = the `circuits` checkout. Not a dependency of `e2e/` or any Noir package. |
| `bb` (fixture generation only) | 5.0.0-nightly, from the separate `5.0.1` Aztec toolchain install | Matches the `bb` build ZKPassport's production circuits were compiled/proved with; required only by `harness/gen-fixtures.sh`, never by test/build commands above. |
