 ...
 * persisting the secret — this script writes a `source`-able env file next to package.json:
 *
 *   deployer-<network>.env      (chmod 600; gitignored via the repo-root `*.env*` rule)
 *
 * so later runs are just:
 *
 ...
 *
 * Safety: if the keys file already exists the script refuses to overwrite it with a
 * different secret — source the file (or delete it) first. Re-running with the same
 * ZKPASSPORT_DEPLOYER_SECRET exported is idempotent.
 *
 * Env:
 *   AZTEC_NODE_URL              node RPC (defaults per network, same as the deploy scripts)
 *   ZKPASSPORT_DEPLOYER_SECRET  reuse an existing secret instead of minting one; the file
 *                               is (re)written either way. On mainnet this secret controls
 *                               real funds and the registry admin role: keep the file safe.
...
async function main() {
  ...
  // admin/oracle role (and on mainnet, lost funds).
  ...
  const lines = [
    ...
    "# KEEP THIS FILE SAFE: the secret controls the registry admin/oracle roles" +
      (NETWORK === "mainnet" ? " and real funds" : "") +
      ".",
