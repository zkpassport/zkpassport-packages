...
import {ProofVerificationParams} from "./lib/Types.sol";
...
contract RootVerifier {
    ...
    // Subverifier mapping
    mapping(bytes32 => SubVerifier) public subverifiers;
    // Counter for the number of subverifiers
    uint256 public subverifierCount;

    // Helper mapping
    mapping(bytes32 => VerifierHelper) public helpers;
    ...
    function verify(ProofVerificationParams calldata params)
        ...
        view
        whenNotPaused
        returns (bool valid, bytes32 uniqueIdentifier, VerifierHelper helper)
    {
        SubVerifier subverifier = subverifiers[params.version];
     ...
     */
    function addSubVerifier(bytes32 version, SubVerifier subVerifier) external onlyAdmin {
    ...
    function updateSubVerifier(bytes32 version, address newSubVerifier) external onlyAdmin {
        ...
        subverifiers[version] = SubVerifier(newSubVerifier);
    ...
    function addHelper(bytes32 version, address newHelper) external onlyAdmin {
        ...
        helpers[version] = VerifierHelper(newHelper);
        helperCount++;
