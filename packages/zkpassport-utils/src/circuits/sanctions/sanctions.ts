...
export class SanctionsBuilder {
  ...
  async getRootHash(): Promise<Buffer> {
    const rootHash = await poseidon2([
      this.documentNumberAndNationalitySMT.root,
      this.nameAndDOBSMT.root,
      this.nameAndYobSMT.root,
    ])
  ...
  async getSanctionsMerkleProofs(
  ...
  ): Promise<{ proofs: SanctionsSparseMerkleTreeProofs; rootHash: string }> {
    ...
    const documentNumberAndNationalityProof = this.documentNumberAndNationalitySMT.createProof(
    ...
    const passportAndNationalityProof: SanctionsMerkleProof = getSanctionsMerkleProof(
      documentNumberAndNationalityProof,
    )
