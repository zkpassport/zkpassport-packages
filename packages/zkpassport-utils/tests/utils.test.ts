import { packBeBytesIntoFields } from "../src/utils"
...
describe("Utils", () => {
  test("packBeBytesIntoFields should return empty array for empty input", () => {
    const result = packBeBytesIntoFields(new Uint8Array([]), 15)
    expect(result).toEqual([])
  })

  test("packBeBytesIntoFields should correctly pack bytes with chunk size (15)", () => {
    const result = packBeBytesIntoFields(new Uint8Array(PUBKEY_BYTES), 15)
    expect(result).toEqual([
  ...
  test("packBeBytesIntoFields should correctly pack bytes with chunk size (31)", () => {
    const result = packBeBytesIntoFields(new Uint8Array(PUBKEY_BYTES), 31)
    expect(result).toEqual([
